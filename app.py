...
def fetch_pilot_name(cid):
    ...
    try:
        pr = requests.get(
            f"https://api.vatsim.net/api/ratings/{cid}/",
        ...
        if r.status_code == 200:
            ...
            for c_ in d.get("controllers", []):
                if str(c_.get("cid")) == str(cid):
                    res = {
                        "online": True,
                        "callsign": c_.get("callsign"),
                        "frequency": c_.get("frequency"),
                        "logon_time": c_.get("logon_time")
                    }
                    cache_set(k, res)
                    return res
    except:
...
def fetch_sessions(cid, start_date):
    ...
    r = requests.get(
        f"https://api.vatsim.net/api/ratings/{cid}/atcsessions/?start={start_date}",
        headers={'Accept': 'application/json'},
        timeout=15
    )

    if r.status_code != 200:
        return {"error": r.status_code, "results": []}
...
def build_heatmap(all_sessions):
    ...
    tm = sum(daily.values())
    ad = sum(1 for v in daily.values() if v > 0)
    st = calc_streaks(daily, today)

    return {
        "weeks": weeks,
        "month_labels": month_labels,
        "total_hhmm": format_hhmm(tm),
        "active_days": ad,
        "current_streak": st["current"],
        "best_streak": st["best"],
        "longest_break": st["longest_break"],
        "has_data": tm > 0
    }
...
def home():
    ...
    if cm >= 7:
        available_periods.append("h2")

    available_quarters = ["q1"]

    if cm >= 4:
        available_quarters.append("q2")

    if cm >= 7:
        available_quarters.append("q3")

    if cm >= 10:
        available_quarters.append("q4")
    ...
    if mode == "h1":
        since, until = datetime(cy, 1, 1), datetime(cy, 6, 30, 23, 59, 59)
    elif mode == "h2":
        since, until = datetime(cy, 7, 1), datetime(cy, 12, 31, 23, 59, 59)
    elif mode == "q1":
        since, until = datetime(cy, 1, 1), datetime(cy, 3, 31, 23, 59, 59)
    elif mode == "q2":
        since, until = datetime(cy, 4, 1), datetime(cy, 6, 30, 23, 59, 59)
    elif mode == "q3":
        since, until = datetime(cy, 7, 1), datetime(cy, 9, 30, 23, 59, 59)
    elif mode == "q4":
        since, until = datetime(cy, 10, 1), datetime(cy, 12, 31, 23, 59, 59)
    else:
        since, until = datetime.now() - timedelta(weeks=13), datetime.now()
    ...
    if sr["error"]:
        return f"API Error: {sr['error']}"

    all_sessions = []

    for s in sr["results"]:
        s = dict(s)
        s["start_dt"] = datetime.fromisoformat(s["start"])
        all_sessions.append(s)

    filtered_data = [
        s for s in all_sessions
        if since <= s["start_dt"] <= until
    ]

    callsign_stats = defaultdict(lambda: {
        "minutes": 0,
        "sessions": 0
    })
    ...
    for s in filtered_data:
        ...
        callsign_stats[cs]["minutes"] += float(
            s.get("minutes_on_callsign", 0)
        )
    ...
    if (
        home_override
        and home_override in groups
        and home_override in active_groups
        and home_override not in excluded
    ):
        home_name = home_override
        home_minutes = totals[home_override]
        home_is_manual = True
    else:
        if totals:
            home_name = max(totals, key=lambda k: totals[k])
            home_minutes = totals[home_name]
    ...
    selectable_homes = sorted([
        n for n in active_groups
        if n not in excluded
    ])

    visiting_minutes = total_all_minutes - home_minutes
    home_meets_50 = (
        home_minutes >= visiting_minutes
        and total_all_minutes > 0
    )
    ...
    home_need = max(
        visiting_minutes - home_minutes,
        0
    )

    home_flags = (
        groups[home_name]["flags"]
        if home_name and home_name in groups
        else []
    )

    period_stats = {
        "total_hhmm": format_hhmm(total_all_minutes),
        "home_name": home_name,
        "home_flags": home_flags,
        "home_hhmm": format_hhmm(home_minutes),
        "visiting_hhmm": format_hhmm(visiting_minutes),
        "home_meets_50": home_meets_50,
        "home_buffer_hhmm": format_hhmm(home_buffer),
        "home_need_hhmm": format_hhmm(home_need),
        "has_data": total_all_minutes > 0,
        "is_manual": home_is_manual
    }
    ...
    if total_all_minutes > 0 and home_name:
        ...
        visiting_need = sum(
            max(0, MIN_MIN - m)
            for m in visiting_divs.values()
        )

        home_min_need = max(
            0,
            MIN_MIN - home_minutes
        )
        ...
        home_for_50_total = max(
            0,
            future_visiting - home_minutes
        )
        ...
        goal_calc = {
            ...
            "visiting_afford_hhmm": format_hhmm(visiting_afford),
            "visiting_afford_min": visiting_afford
        }
    ...
    def future_expiry(sessions, prefixes, min_hours):
        ...
        rel = [
            s for s in sessions
            if s["callsign"].upper().startswith(prefixes)
        ]

        times = [
            (
                s["start_dt"].date(),
                float(s["minutes_on_callsign"])
            )
            for s in rel
        ]

        td = datetime.utcnow().date()
        ed = td + timedelta(weeks=52)

        ct = sum(
            m for d, m in times
            if td - timedelta(weeks=13) <= d <= td
        )
    ...
    for name, info in groups.items():
        ...
        pct = (
            mins / total_all_minutes * 100
            if total_all_minutes
            else 0
        )
        ...
        ld = [
            s["start_dt"]
            for s in all_sessions
            if s.get("callsign", "").upper().startswith(
                info["prefixes"]
            )
        ]
        ...
        results.append({
            ...
            "minimum": info["min_hours"],
            "minimum_hhmm": format_hhmm(
                info["min_hours"] * 60
            ),
            "percent": round(pct, 1),
            "status": st,
            "expiry": exp,
            "last_controlled": lct,
            "last_controlled_days": da
        })

    def esk(x):
        e = x["expiry"]

        if e == "Will not expire":
            return datetime.max

        if e in ["Not fulfilled", "Half/Quarter mode"]:
            return datetime.min
    ...
    if sort_by == "expiry":
        results.sort(key=esk)
    elif sort_by == "last":
        results.sort(
            key=lambda x: x["last_controlled_days"]
        )
    else:
        results.sort(
