...
GROUPS = {
    ...
    "VATSSA": {"prefixes": ("FA","FI","FM","FQ","FV","FB","FY","FN","FL","FW","HT","HB","HR","FZ","FC","HK","DG","DB","DU","GK","GI","GV","DR","FT","FS"), "min_hours": 3, "flags": ["za"]},
    "vZMA ARTCC": {"prefixes": ("TPA","RSW","PBI","MIA"), "min_hours": 3, "flags": ["us"]},
    "Atlanta ARTCC": {"prefixes": ("CLT","ATL"), "min_hours": 3, "flags": ["us"]},
    "VATSIM North East Africa": {"prefixes": ("HS","HJ","HA","HH","HC","HD"), "min_hours": 3, "flags": ["sd"]},
    ...
    "VATAdria": {"prefixes": ("ADR","LJ","LQ","LD","LY","LW","LA"), "min_hours": 3, "flags": ["si"]},
    ...
    "VATSIM Scandinavia": {"prefixes": ("BI","EK","EN","EF","ES"), "min_hours": 3, "flags": ["se"]},
    ...
    "Caucasus ACC": {"prefixes": ("UB","UD","UG","UR"), "min_hours": 3, "flags": ["az"]},
    "vACC Ukraine": {"prefixes": ("UK",), "min_hours": 3, "flags": ["ua"]},
}
EDYY_DUTCH = {"EDYY_D_CTR"}
EDYY_BELUX = {"EDYY_N_CTR","EDYY_K_CTR","EDYY_L_CTR","EDYY_O_CTR","EDYY_BRU_CTR","EDYY_BR_CTR","EDYY_O__CTR"}
...
def home():
    ...
    if home_override and home_override in GROUPS and home_override in active_groups and home_override not in excluded:
        home_name = home_override; home_minutes = totals[home_override]; home_is_manual = True
    else:
        if totals:
    ...
    if total_all_minutes > 0 and home_name:
        ...
        visiting_afford = max(0, home_minutes - future_visiting) if (home_add_total == 0 and visiting_need == 0) else 0
    ...
    for name, info in GROUPS.items():
        ...
        exp = future_expiry(filtered_data, name, info["min_hours"])
        ld = [s["start_dt"] for s in all_sessions if classify(s.get("callsign","").upper()) == name]
        if ld:
